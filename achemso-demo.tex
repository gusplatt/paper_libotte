...
\altaffiliation{Institute for Pure and Applied Mathematics, Rio de Janeiro, Brazil}
...
%
%\noindent where $ h > 0 $ represents the stepsize and $ t \left( H^{\prime}\left( u_{i} \right) \right) $ is an unitary vector tangent to the solution curve at the point $ u_{i} $, obtained by QR decomposition. The stepsize must be arbitrarily small in order to obtain a reasonable approximation. Also, if $ u_{i} $ is sufficiently close to the curve, then the predicted point $ v_{i + 1} $ will also be sufficiently close to the curve and the minimization problem will have a unique solution, which will be refined in the corrector step. In turn, the scheme used in the corrector step is the Newton's method.
